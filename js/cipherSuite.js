var cipherSuite = {
	...
	aesEncrypt: function (input, key, IV, validateKey) {
		try {
			...
			if (_validateKey) {
				//Se añaden parámetros para poder validar la llave
	...
	aesDecrypt: function (encrypted, key, IV, validateKey) {
		try {
			...
			if (resultDecrypted == false) {
				...
				if (_validateKey) {
					//Se verifica si se va a validar o no la llave.
					try {
	 ...
	 * @param {Object|String} [certificate] - Certificado  donde se obtendrá la llave pública para cifrar. (Buffer)  ó Base64 del Certificado, si viene este campo se ignorará publicKeyPEM
	 * @param {String} [publicKeyPEM] - Llave pública para generar el cifrado en formato PEM
	 * <pre>
	 * Ejemplo fs.readFile(pathPem, function (error, cer){
	 ...
	 */
	rsaEncrypt: function (plainText, certificate, publicKeyPEM) {
		try {
			var result = {
				data: {
					encryptedText: null
			...
			if (!(plainText && (certificate || publicKeyPEM))) {
				result.message = 'Error en los parámetros de entrada (pre)'
				return result
			}
			if (!(typeof plainText === 'string' && (typeof certificate === 'object' || typeof certificate === 'string' || typeof publicKeyPEM === 'string'))) {
				result.message = 'Error en los parámetros de entrada (pret)'
				return result
			}
			let _this = cipherSuite
			let publicKey = null
			if (certificate) {
				var _certificate = _this.getForgeObjectFromCer(certificate)
				if (_certificate) {
					publicKey = _certificate.publicKey
				} else {
			...
			if (!publicKey) {
				result.message = 'Ocurrió un error al obtener la llave pública'
				return result
			}
    ...
    /**
	 * @function rsaDecryptFromDecryptedPem
	 * @name module:Components/cipherSuite#rsaDecryptFromDecryptedPem
	 * @public
	 * @description -  Función para descifrar con el algoritmo RSA con el padding RSAES PKCS#1 v1.5 a partir de una llave descifrada
	 * @param {String} encryptedText - Cadena de caracteres a descifrar.
	 * @param {Object/String} privateKey - Llave para descifrar cadena cifrada. .key en formato DER
	 * <pre>
	 * Ejemplo fs.readFile(pathPem, function (error, key){
	 * var privateKey= key
	 * },'utf8')
	 * </pre>
	 * @returns {rsaDecryptResult} - Resultado del descifrado. {@link module:Components/cipherSuite#rsaDecryptFromDecryptedPemResult}
	 */
	rsaDecryptFromDecryptedPem: function(encryptedText, privateKeyPem) {
		try {
			console.log('rsaDecryptFromDecryptedPem')
			var result = {
				data: {
					plainText: null
				},
			...
			if (!(encryptedText && privateKeyPem)) {
				result.message = 'Error en los parámetros de entrada para descrifrar rdd'
				return result
			}
			if (!(typeof encryptedText === 'string' && typeof privateKeyPem === 'string')) {
				result.message = 'Error en los parámetros de entrada para descrifrar (rdd)'
				return result
			}
			var _key = forge.pki.privateKeyFromPem(privateKeyPem)
			if (_key) {
				var plainText = _key.decrypt(encryptedText)
				result.data.plainText = plainText
				result.code = 1
